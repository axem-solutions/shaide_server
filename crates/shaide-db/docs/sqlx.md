# Using SQLX with shaide

shaide uses SQLX for its database, which comes with several advantages:
...
- A local copy of our db schema is maintained in `crates/shaide-db/schema.sqlite`
...
```
rm crates/shaide-db/schema.sqlite
cargo sqlx db setup --source crates/shaide-db/migrations
```

To create a new migration:

```
cargo sqlx migrate add --source crates/shaide-db/migrations -r -s <migration name>
```
