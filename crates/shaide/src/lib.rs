...
/// Creates the admin user when it is missing, and verifies it is really an admin when it is not.
...
/// The 404 fallback and the outermost middleware chain every served request passes through.
///
/// Kept separate from [`api_router`] so the integration-test harness mounts the exact same
/// error-shaping, header-forwarding and logging layers the deployed server does, without the
/// layers that need something outside the process (the reverse proxy and the Prometheus
/// recorder).
pub fn with_api_middleware(router: Router) -> Router {
    router
...
pub async fn start_server() {
    ...
    let app = with_api_middleware(app);
